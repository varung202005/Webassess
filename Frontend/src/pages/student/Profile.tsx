...
export default function Profile() {
  ...
  return <StudentLayout><PageState loading={portal.isLoading} error={portal.error}>
    ...
    <div className="profile-card">
      <aside className="profile-summary">{profile?.profile_photo ? <img className="avatar" src={profile.profile_photo} alt="" /> : <div className="avatar">{initials(profile?.full_name)}</div>}<h2>{profile?.full_name}</h2><p>{profile?.email}</p><p>{profile?.roll_number || "Roll number not set"}</p></aside>
      <form className="panel" onSubmit={submit}><div className="panel-header"><h2>Student Information</h2></div><div className="panel-body form-grid">
        ...
        <div className="form-field full"><button className="btn btn-primary" disabled={update.isPending} type="submit">{update.isPending ? "Saving..." : "Update Profile"}</button></div>
      </div></form>
    </div>
