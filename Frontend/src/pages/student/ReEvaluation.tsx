...
export default function ReEvaluation() {
  ...
  return <StudentLayout><PageState loading={portal.isLoading} error={portal.error}>
    ...
    <section className="panel" style={{ marginBottom: 16 }}><div className="panel-header"><h2>Eligible Results</h2></div><div className="panel-body">
    ...
    {selected && <div className="modal-backdrop"><section className="modal"><div className="modal-header"><h2>Request Re-Evaluation</h2><button onClick={() => setSelected(null)}><i className="ti ti-x" /></button></div><div className="modal-body"><p style={{ marginBottom: 12 }}>{selected.exam.title} · {selected.total_score}/{selected.max_score}</p><textarea className="textarea" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explain the specific grading concern (minimum 10 characters)" /></div><div className="modal-footer"><button className="btn btn-secondary" onClick={() => setSelected(null)}>Cancel</button><button className="btn btn-primary" disabled={reason.trim().length < 10 || request.isPending} onClick={submit}>{request.isPending ? "Submitting..." : "Submit Request"}</button></div></section></div>}
