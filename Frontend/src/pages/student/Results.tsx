...
export default function Results() {
  ...
  return <StudentLayout><PageState loading={portal.isLoading} error={portal.error}>
    ...
    <div className="filter-panel"><input className="field" placeholder="Search results" value={search} onChange={(event) => setSearch(event.target.value)} /><select className="select" value={semester} onChange={(event) => setSemester(event.target.value)}><option value="ALL">All semesters</option>{Array.from({ length: 12 }, (_, index) => index + 1).map((item) => <option value={item} key={item}>Semester {item}</option>)}</select></div>
