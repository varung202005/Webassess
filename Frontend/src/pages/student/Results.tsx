...
export default function Results() {
  ...
  return <StudentLayout><PageState loading={portal.isLoading} error={portal.error}>
    ...
    <div className="filter-panel compact-filter-panel"><div className="filter-control search-control"><i className="ti ti-search" /><label className="visually-hidden" htmlFor="results-search">Search results</label><input id="results-search" className="field" placeholder="Search results" value={search} onChange={(event) => setSearch(event.target.value)} /></div><div className="filter-control"><i className="ti ti-school" /><label className="visually-hidden" htmlFor="results-semester">Semester</label><select id="results-semester" className="select" value={semester} onChange={(event) => setSemester(event.target.value)}><option value="ALL">All semesters</option>{Array.from({ length: 12 }, (_, index) => index + 1).map((item) => <option value={item} key={item}>Semester {item}</option>)}</select></div></div>
